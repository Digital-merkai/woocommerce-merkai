...
(( $ ) => {
    ...
    const register = (evt) => {
        ...
        if(!$('#user_login').val() && !$('#user_email').val()) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    'action': 'merkai_ajax_registration',
    ...
    const loginaction = (evt) => {
        ...
        if(!$('#user_login').val() && !$('#user_pass').val()) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    'action': 'merkai_ajax_login',
                    'nonce': $('#loginsecurity').val(),
                    'password': $('#user_pass').val(),
                    'username': $('#user_name').val(),
                    'remember': $('#merkai_rememberme').val(),
                },
                success: function(data){
    ...
    $(document).ready(function() {
        ...
        $(document).on( "updated_checkout", function() {
            ...
            const merkai_auth_block = $('#merkai_auth_block').length
            const place_orderButton = $('#place_order');

            const payment_hidden = ($('.payment_box.payment_method_merkai').is(":hidden"));
            const activation_block = $('.payment_box.payment_method_merkai .merkai').length;
