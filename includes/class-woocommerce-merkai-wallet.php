...
/**
 * MerkaiWallet class for managing digital wallets.
 *
 * @author Merkai inc.
 * @version 1.0
 * @link https://merkai.com/
 */
class Woocommerce_Merkai_Wallet {
    ...
    private function sendRequest(string $method, string $url, string $body = "", bool $simple = false): array {
        ...
        $WPPPG = new Woocommerce_Merkai_Payment_Gateway;
    ...
    */
    ...
    {
        $user_merkai_wallet = $this->getWalletById($walletId);
        if($user_merkai_wallet['status_code'] === 200) {
            $json_response = json_decode($user_merkai_wallet['response']);
            return [
        ...
        return [
            ...
            'bonuses' => 0,
            'number' => 0,
            'status' => $user_merkai_wallet['response'],
            'code' => $user_merkai_wallet['status_code'],
        ];
    }
    ...
    */
    ...
    public function getEnvironmentStructure(): array | null
    {
